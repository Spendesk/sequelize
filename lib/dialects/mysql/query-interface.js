...
var removeColumn = function (tableName, columnName, options) {
  var self = this;
  options = options || {};
  var table = tableName.tableName ? tableName : { tableName: tableName };
  table.database = self.sequelize.config.database;

  return self.sequelize.query(
      self.QueryGenerator.getForeignKeyQuery(table, columnName),
